  //基础fetch实例
//前置拼接urllet api = '****';//处理promise和fetch的兼容性以及引入
...
let Fetch = (url, option = {}) => {
  ...
  option.headers['token'] = `${window.localStorage.getItem('token')}`;  const m = (option.method || '').toLocaleLowerCase();  // get query format
  if (m == 'get') {    if (option.query) {
  ...
  if (m === 'post' || m === 'put' || m === 'delete') {
    ...
    fetch(api + url, option)
      .then(response => {
        status = response.status;        if (response.status >= 500) {
          Fetch.systemError && Fetch.systemError('系统错误');
        }        return response;
      })
      .then(parseJSON)
      .then(response => {
        response.status = status;        if (response.status >= 401) {          if (response.state == 8888) {            //登陆超时返回状态吗
...
//  let res = await this.Fetch('url', {        method: 'get',
//  query
// });
